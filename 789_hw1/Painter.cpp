...
SoSeparator* Painter::getShapeSep(Mesh* mesh)
{
	...
	mat->diffuseColor.setValue(0.5, 0.5, 0.5); //paint all vertices with this color
	...
	if (youWantToPaintEachVertexDifferently)
		for (int i = 0; i < (int)mesh->verts.size(); i++) //i = 0 obj->color above overwritten here
			mat->diffuseColor.set1Value(i, 1, 0, 0); //vert color according to its x-y-z coord (for mesh1) and to the transferred color (for mesh2)
